...
def subset_table_by_vpu(table: str, vpu: str, hydrofabric: Path, subset_gpkg_name: Path) -> None:
    ...
    if table == "network":
        ...
        logger.info(
            "Removing %d network entries that are not in flowpaths or nexuses", len(bad_ids)
        )
...
def get_table_crs_short(gpkg: str | Path, table: str) -> str:
    """
    Gets the CRS of the specified table in the specified geopackage as a short string. e.g.
    EPSG:5070
...
def get_table_crs(gpkg: str, table: str) -> str:
    ...
    con = sqlite3.connect(gpkg)
    sql_query = (
        "SELECT g.definition FROM gpkg_geometry_columns AS c JOIN gpkg_spatial_ref_sys AS g ON "
        + f"c.srs_id = g.srs_id WHERE c.table_name = '{table}'"
    )
    crs = con.execute(sql_query).fetchone()[0]
    con.close()
    return crs
...
def get_cat_from_gage_id(gage_id: str, gpkg: Path = FilePaths.conus_hydrofabric) -> str:
    ...
    with sqlite3.connect(gpkg) as con:
        ...
        if len(result) == 0:
            ...
            raise IndexError(f"Could not find a waterbody for gage {gage_id}", gage_id)
        if len(result) > 1:
...
def get_cat_to_nhd_feature_id(gpkg: Path = FilePaths.conus_hydrofabric) -> Dict[str, int]:
    ...
    if len(tables) == 0:
        raise IndexError(
            f"No source data found for NHD ID in {available_tables}, expected one of "
            + f"{possible_tables}"
        )

    table_name = list(tables)[0]
    sql_query = (
        f"SELECT divide_id, hf_id FROM {table_name} WHERE divide_id IS NOT NULL AND hf_id IS NOT "
        + "NULL ORDER BY hf_hydroseq DESC"
    )
