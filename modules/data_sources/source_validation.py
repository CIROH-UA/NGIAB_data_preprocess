...
def download_snow17_attributes():
    ...
    if not FilePaths.snow17_attributes.exists() or headers.get("ETag", "") != local_headers.get(
        "ETag", ""
    ):
        download_from_s3(
            FilePaths.snow17_attributes,
            bucket=S3_BUCKET,
            key=s3_key,
        )
        with open(FilePaths.snow17_attributes.with_suffix(".log"), "w") as f:
            json.dump(dict(headers), f)

def download_sacsma_attributes():
    ...
    if not FilePaths.sacsma_attributes.exists() or headers.get("ETag", "") != local_headers.get(
        "ETag", ""
    ):
        download_from_s3(
            FilePaths.sacsma_attributes,
            bucket=S3_BUCKET,
            key=s3_key,
        )
        with open(FilePaths.sacsma_attributes.with_suffix(".log"), "w") as f:
            json.dump(dict(headers), f)
